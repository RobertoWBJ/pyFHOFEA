...
class Pre_processing():
    ...
    def mesh_statistics(self):
        print(f'\nQuantity of nodes: {len(self.point)}')
        print(f'Quantity of elements: {len(self.cells["triangle"])}\n')
        pass
...
class Processing():
    ...
    def __init__(self, inputs: Pre_processing):
        ...
        self.force_result_matrix = np.zeros(self.inputs.DOF_structure)
        self.K_matrix = np.zeros((self.inputs.DOF_structure, self.inputs.DOF_structure))
        self.stress_matrix = np.zeros((len(self.elements), 10))
    ...
    def solve(self):
        Processing.K_matrix_assemblage(self)
    ...
    def get_all_results(self):
        self.force_result_matrix = self.K_matrix @ self.displacement_matrix
        ...
        for pos, nodes in enumerate(self.elements):
            ...
            strain_X = 1/self.inputs.young_modulus*(sigma_X - self.inputs.poisson_coef*sigma_Y)
            strain_Y = 1/self.inputs.young_modulus*(sigma_Y - self.inputs.poisson_coef*sigma_X)
            gamma_XY = 2*tau_XY*(1 + self.inputs.poisson_coef)/self.inputs.young_modulus
            ...
            self.stress_matrix[pos, :] = np.array([strain_X, strain_Y, gamma_XY,
...
class Post_processing():
    ...
    def __init__(self, pre_data: Pre_processing, pro_data: Processing):
        ...
        self.force_matrix = pro_data.force_result_matrix
    ...
    def reaction_values(self, bc:str):
        geom_type, element_reference = self.get_nodes_from_physical_group(bc)
        unique_nodes_id_list = np.unique(np.array(element_reference).flatten())

        Rx = 0
        Ry = 0

        for i in unique_nodes_id_list:
            Rx += self.force_matrix[2*i]
            Ry += self.force_matrix[2*i + 1]

        reactions = {bc:[Rx, Ry]}

        return reactions

    def plot_func(self, contour, title, scale=0, bc=None):

        if scale != 0:
            displaced_x = np.copy(self.x)
            displaced_y = np.copy(self.y)

            for i in range(len(displaced_x)):
                displaced_x[i] += self.displacement_matrix[2*i]*scale
                displaced_y[i] += self.displacement_matrix[2*i + 1]*scale

            triangulation = tri.Triangulation(displaced_x, displaced_y, self.elements)
        else:
            triangulation = tri.Triangulation(self.x, self.y, self.elements)
        ...
        if bc is not None:
            plt.triplot(triangulation, color='black', linewidth=0.1)
            triangulation, contour = Post_processing.get_local_plot(self, bc, triangulation, contour)

        if abs(np.std(contour) / np.mean(contour)) <= 0.0001:
            contour = np.ones(len(contour)) * np.mean(contour)
            ticks = np.ones(1) * np.mean(contour)
        else:
            ticks = np.linspace(min(contour), max(contour), 10)

        tpc = plt.tripcolor(triangulation, facecolors=contour, edgecolors='none', cmap=self.color_contour)
        cbar = plt.colorbar(tpc, ticks=ticks, format='%.03e')
        cbar.ax.tick_params(labelsize=20)
        plt.show()
        pass

    def get_local_plot(self, bc, triangulation, contour):
        geom_type, element_reference, first_index, last_index = self.get_nodes_from_physical_group(bc, output_index=True)

        new_element_reference = np.copy(element_reference)
        ...
        if geom_type == 'triangle':
            for i, node in enumerate(element_reference):
                base_matrix[i * 3, 1:] = node[0], triangulation.x[int(node[0])], triangulation.y[int(node[0])]
                base_matrix[i * 3 + 1, 1:] = node[1], triangulation.x[int(node[1])], triangulation.y[int(node[1])]
                base_matrix[i * 3 + 2, 1:] = node[2], triangulation.x[int(node[2])], triangulation.y[int(node[2])]
        else:
        ...
        for new_ref, old_ref in base_matrix[:, 0:2]:
            new_element_reference[new_element_reference == old_ref] = new_ref

        triangulation = tri.Triangulation(base_matrix[:, 2], base_matrix[:, 3], new_element_reference)
        return triangulation, contour

    def strain_X(self, scale: int = 0, bc=None):
        Post_processing.plot_func(self, contour=self.stress_results[:, 0], title='Strain X', scale=scale, bc=bc)
        pass

    def strain_Y(self, scale: int = 0, bc=None):
        Post_processing.plot_func(self, contour=self.stress_results[:, 1], title='Strain Y', scale=scale, bc=bc)
        pass

    def gamma_XY(self, scale: int = 0, bc=None):
        Post_processing.plot_func(self, contour=self.stress_results[:, 2], title='Gamma XY', scale=scale, bc=bc)
        pass

    def stress_X(self, scale: int = 0, bc=None):
        Post_processing.plot_func(self, contour=self.stress_results[:, 3], title='Stress X', scale=scale, bc=bc)
        pass

    def stress_Y(self, scale: int = 0, bc=None):
        Post_processing.plot_func(self, contour=self.stress_results[:, 4], title='Stress Y', scale=scale, bc=bc)
        pass

    def stress_XY(self, scale: int = 0, bc=None):
        Post_processing.plot_func(self, contour=self.stress_results[:, 5], title='Stress XY', scale=scale, bc=bc)
        pass

    def max_principal(self, scale: int = 0, bc=None):
        Post_processing.plot_func(self, contour=self.stress_results[:, 6], title='Maximum Principal', scale=scale, bc=bc)
        pass

    def min_principal(self, scale: int = 0, bc=None):
        Post_processing.plot_func(self, contour=self.stress_results[:, 7], title='Minimum Principal', scale=scale, bc=bc)
        pass

    def max_plane_shear(self, scale: int = 0, bc=None):
        Post_processing.plot_func(self, contour=self.stress_results[:, 8], title='Maximum Plane Shear', scale=scale, bc=bc)
        pass

    def von_mises(self, scale: int = 0, bc=None):
        Post_processing.plot_func(self, contour=self.stress_results[:, 9], title='von Mises', scale=scale, bc=bc)
        pass